...
- 所有新增或修改的 Java 类、内部类、接口、记录类、枚举、字段、常量、构造方法和方法都必须添加简明准确的中文 JavaDoc 注释
- 实体类、DTO、配置类及其字段必须说明用途、数据含义、单位、默认值或取值约束；字段注释必须紧邻字段声明
- 测试类、测试方法和测试辅助方法必须说明被验证的场景，配置文件和模板中的新增配置项或数据绑定区域必须添加中文注释
- JavaDoc 必须放在该声明全部注解的上方，禁止将注释放在 `@Data`、`@Builder`、`@Test` 等注解之后
- 注释应解释职责、业务含义和约束，不重复描述显而易见的语法；修改代码逻辑或字段含义时必须同步更新注释
